...
CROP_ORDER = ["masur", "bajra", "moong", "cauliflower"]

st.set_page_config(page_title="WPI Forecasting Dashboard", layout="wide")
...
available_crops = sorted(models.keys(), key=lambda x: CROP_ORDER.index(x) if x in CROP_ORDER else len(CROP_ORDER))
selected_crop = st.sidebar.selectbox("Select Commodity", available_crops)
forecast_months = st.sidebar.slider("Forecast Horizon (months)", min_value=6, max_value=24, value=12)
...
st.dataframe(future_forecast.style.format({"yhat": "{:.2f}", "yhat_lower": "{:.2f}", "yhat_upper": "{:.2f}"}))

csv = future_forecast.to_csv(index=False).encode("utf-8")
st.download_button(label="Download Forecast CSV", data=csv, file_name=f"{selected_crop}_forecast.csv", mime="text/csv")
st.caption("Note: WPI data is published with a delay and the most recent values may be provisional. Forecasts are forward-looking estimates based on historical trends and seasonality.")